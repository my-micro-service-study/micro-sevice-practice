...
CREATE TABLE `meite_blacklist` (
    `ID` int(12) NOT NULL AUTO_INCREMENT COMMENT 'user_id',
    `ip_addres` varchar(11) NOT NULL COMMENT 'IP地址',
    `restriction_type` int(2) DEFAULT NULL COMMENT '是否限制 1限制  0不限制',
    `availability` int(2) DEFAULT NULL COMMENT '是否可用 1可用  0不可用',
    `REVISION` int(11) DEFAULT NULL COMMENT '版本',
    `CREATED_BY` varchar(32) DEFAULT NULL COMMENT '创建人',
    `CREATED_TIME` datetime DEFAULT NULL COMMENT '创建时间',
    `UPDATED_BY` varchar(32) DEFAULT NULL COMMENT '更新人',
    `UPDATED_TIME` datetime DEFAULT NULL COMMENT '更新时间',
    UNIQUE KEY `MOBILE_UNIQUE` (`ID`)
    ) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8 COMMENT='使用网关限制ip黑名单表';

DROP TABLE IF EXISTS `meite_app_info`;
CREATE TABLE `meite_app_info` (
    `app_id` varchar(100) NOT NULL COMMENT 'app_id',
    `app_secret` varchar(100) NOT NULL COMMENT 'app_id',
    `availability` int(2) DEFAULT NULL COMMENT '是否可用 1可用  0不可用',
    `REVISION` int(11) DEFAULT NULL COMMENT '版本',
    `CREATED_BY` varchar(32) DEFAULT NULL COMMENT '创建人',
    `CREATED_TIME` datetime DEFAULT NULL COMMENT '创建时间',
    ...
    ) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8 COMMENT='app info 信息';